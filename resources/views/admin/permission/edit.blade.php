...
<article class="cl pd-20">
	<form action="{{adm_url('permission/edit')}}" method="post" class="form form-horizontal" id="form-admin-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$sort}}" placeholder="排序" id="sort" name="sort">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
				<select class="select" name="pid" size="1">
					...
					@foreach($list as $v)
					<option @if($pid==$v['id']) selected @endif value="{{$v['id']}}">@for($i=0;$i<$v['level']*5;$i++) - @endfor {{$v['title']}}</option>
					@endforeach
				</select>
				</span> </div>
		</div>
